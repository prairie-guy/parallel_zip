...
def execute_command(commands, dry_run, verbose):
    ...
    # if dry_run: return [], processed_commands
    # if verbose: proc = subprocess.run(["rust-parallel", ":::", *processed_commands], capture_output=True, text=True)
    # else: proc = subprocess.run(["rust-parallel", ":::", *processed_commands], capture_output=True, text=True)
